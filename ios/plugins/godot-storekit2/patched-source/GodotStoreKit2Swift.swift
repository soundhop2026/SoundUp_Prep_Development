...
@unchecked Sendable
{
	...
	public func getProductInfo(productId: NSString) async throws -> ProductInfo {
		...
		// PATCHED (SoundHop, 2026-08-27): the original code branched on
		// `#available(iOS 18.4, *)` to use the newer `product.currentEntitlements`
		// (plural) async sequence, falling back to `product.currentEntitlement`
		// (singular) otherwise. `#available` only defers *execution* to runtime —
		// Swift still fully compiles and links the 18.4-only branch for every
		// build, and doing so requires the `AsyncIteratorProtocol.next(isolation:)`
		// witness, which has no back-deployment shim below iOS 18.x. That made a
		// clean archive fail to link for any deployment target under ~18.5,
		// even though the branch could never execute there anyway.
		// SoundHop's minimum target is 15.0, so the 18.4 branch is unreachable
		// dead code for us. Removed it and kept only the singular-entitlement
		// path (identical to what every pre-18.4 device already used), so
		// behavior for every device this app actually supports is unchanged.
